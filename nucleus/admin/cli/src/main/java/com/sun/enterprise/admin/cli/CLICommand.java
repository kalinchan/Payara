...
/**
 * Base class for a CLI command. An instance of a subclass of this class is created using the getCommand method with the name of the command and the information
 * about its environment.
 * <p>
 * A command is executed with a list of arguments using the execute method. The implementation of the execute method in this class saves the arguments in the
 * protected argv field, then calls the following protected methods in order: prepare, parse, validate, and executeCommand. A subclass must implement the
 * prepare method to initialize the metadata that specified the valid options for the command, and the executeCommand method to actually perform the command.
 * The parse and validate method may also be overridden if needed. Or, the subclass may override the execute method and provide the complete implementation for
 * the command, including option parsing.
 *
 * @author Bill Shannon
 */
@Contract
...
public abstract class CLICommand implements PostConstruct {
    ...
    /**
     * The options parsed from the command line. Initialized by the parse method. The keys are the parameter names from the command model, not the "forced to
     * all lower case" names that are presented to the user.
     */
    ...
    /**
     * Constructor used by subclasses to save the name, program options, and environment information into corresponding protected fields. Finally, this
     * constructor calls the initializeLogger method.
     *
     * @param name
     * @param programOpts
     * @param env
    ...
    /**
     * Execute this command with the given arguemnts. The implementation in this class saves the passed arguments in the argv field and calls the
     * initializePasswords method. Then it calls the prepare, parse, and validate methods, finally returning the result of calling the executeCommand method.
    ...
    /**
     * Subclasses can override this method to supply additional or different options that should be part of the usage text. Most commands will never need to do
     * this, but the create-domain command uses it to include the --user option as a required option.
     *
     * @return
     */
    ...
    /**
     * Get the usage text for the command. This usage text shows the details of the command options but does not show the details for the subcommand options.
     * The subcommand argument is used to fill in the subcommand name in the usage text.
     *
    ...
    public static String quote(String value) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (c == '"' || c == '\\' || c == '\r' || c == '\n') {
                ...
                sb.append('"');
                return sb.toString();
            } else if (c <= 040 || c >= 0177) // These characters cause the string to be quoted
            {
                needQuoting = true;
            }
        }
    ...
    /**
     * If the program options haven't already been set, parse them on the command line and remove them from the command line. Subclasses should call this method
     * in their prepare method after initializing commandOpts (so usage is available on failure) if they want to allow program options after the command name.
     * Currently RemoteCommand does this, as well as the local commands that also need to talk to the server.
     *
     * @throws CommandException
     */
    ...
    /**
     * Initialise the passwords field based on the password file specified in the program options, and initialise the program option's password if available in
     * the password file.
     *
     * @throws CommandException
     */
    ...
    /**
     * The parse method sets the options and operands fields based on the content of the command line arguments. If the program options say this is a help
     * request, we set options and operands as if "--help" had been specified.
     *
     * @throws CommandException if execution of the command fails
     * @throws CommandValidationException if there's something wrong with the options or arguments
     */
    protected void parse() throws CommandException {
        ...
        if (logger.isLoggable(Level.FINER)) {
            logger.log(Level.FINER, "params: {0}", options);
            logger.log(Level.FINER, "operands: {0}", operands);
        }
    }

    /**
     * Check if the current request is a help request, either because --help was specified as a program option or a command option. If so, get the man page
     * using the getManPage method, copy the content to System.out, and return true. Otherwise return false. Subclasses may override this method to perform a
     * different check or to use a different method to display the man page. If this method returns true, the validate and executeCommand methods won't be
     * called.
     *
     * @return
     * @throws CommandException
    ...
    /**
     * The prevalidate method supplies missing options from the environment. It also supplies passwords from the password file or prompts for them if
     * interactive.
     *
     * @throws CommandException           if execution of the command fails
    ...
    /**
     * The validate method can be used by a subclass to validate that the type and quantity of parameters and operands matches the requirements for this
     * command.
     *
     * @throws CommandException if execution of the command fails
     * @throws CommandValidationException if there's something wrong with the options or arguments
    ...
    /**
     * Get a password for the given option. First, look in the passwords map. If found, return it. If not found, and not required, return null; If not
     * interactive, return null. Otherwise, prompt for the password. If create is true, prompt twice and compare the two values to make sure they're the same.
     * If the password meets other validity criteria (i.e., length) returns the password. If defaultPassword is not null, "Enter" selects this default password,
     * which is returned.
     *
     * @param opt
    ...
    /**
     * Get option values, that might come from the command line or from the environment. Return the default value for the option if not otherwise specified.
     * This method works with options for with multiple() is true.
     *
     * @param name
     * @return
     */
