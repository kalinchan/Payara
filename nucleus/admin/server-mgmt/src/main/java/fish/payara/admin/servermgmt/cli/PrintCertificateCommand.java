...
public class PrintCertificateCommand extends CLICommand {
    ...
    protected void validate() throws CommandException, CommandValidationException {
        ...
        if (isDerEncodedFile()) {
            ...
            final String password = getPassword("keystorePassword", "Keystore Password", null, false);
    ...
    private X509Certificate getCertificateFromDerFile() throws CommandException, IOException {
        final FileInputStream inputStream = new FileInputStream(this.derFile);
        try {
            final CertificateFactory cf = CertificateFactory.getInstance("X.509");
            final Collection<? extends Certificate> chain = cf.generateCertificates(inputStream);
            if (chain.isEmpty()) {
                return null;
            }
            final Certificate certificate = chain.iterator().next();
            return getX509Certificate(certificate);
    ...
    private X509Certificate getCertificateFromKeystore() throws CommandException, IOException {
        final FileInputStream inputStream = new FileInputStream(this.keystoreFile);
        try {
            ...
            final Certificate certificate = keystore.getCertificate(this.certificateAlias);
            return getX509Certificate(certificate);
        } catch (final Exception e) {
    ...
    private static X509Certificate getX509Certificate(final Certificate certificate) throws Exception {
